...
namespace ArcGISRuntime.Samples.DesktopViewer.Utils
{
    public class GraphUtils
    {
        ...
        public Task TspAlgorithm(CancellationToken token, int population, int maxgenerations, List<GraphVertexClass> graphVertexList, GraphVertexClass startingVertex = null)
        {
        ...
        private double _oldFitness = 0.0;
        ...
        private void tsp_newCalcStarted(object sender, TspCalcEventArgs e)
        {
            ...
            {
                this.GetDependencyResolver().Resolve<IMessageMediator>().SendMessage(string.Format("Finding tours with better fitness {0}/{1}", e.OnGoing, e.Max), "UpdateStatusBar");
            }
            else
            {
